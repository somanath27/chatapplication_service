...
module.exports.accessChat = asyncHandler(async (req, res) => {
    ...
    isChat = isChat.map((chat) => {
        chat.users = Array.from(
            new Set(chat.users.map((user) => user._id.toString()))
        ).map((id) => chat.users.find((user) => user._id.toString() === id));
        return chat;
    });

    // Remove duplicates from isChat based on _id
    isChat = isChat.filter(
        (chat, index, self) =>
            index ===
            self.findIndex((c) => c._id.toString() === chat._id.toString())
    );
...
module.exports.createGroupChat = asyncHandler(async (req, res) => {
    ...
    // to add all user also incluing self user
    users.push(req.user);
